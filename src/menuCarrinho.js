...
function atualizarInformacaoProduto(idProduto) {
  ...
  document.getElementById(
    `preco-${idProduto}`
  ).innerText = `${precoBr.toLocaleString("pt-BR", {
    style: "currency",
    currency: "BRL",
  })}`;
}
...
export function desenharProdutoNoCarrinho(idProduto) {
  ...
  const precoBr = produto.preco * idsProdutoCarrinhoQuandtidade[idProduto];
  const cardPodutoCarrinho = `<button id="remover-item-${
    produto.id
        ...
        }" class="h-14 rounded-lg" alt="">
        <div class="p-2 flex flex-col justify-between">
            <p class="text-teal-900 text-sm">${produto.nome}</p>
            <p class="text-green-700 text-lg" id="preco-${
              produto.id
            }">${precoBr.toLocaleString("pt-BR", {
    style: "currency",
    currency: "BRL",
  })}</p>
        </div>
    ...
    idsProdutoCarrinhoQuandtidade[produto.id]
  }</p>
...
export function atualizarPrecoCarrinho() {
  ...
  for (const idProdutoNoCarrinho in idsProdutoCarrinhoQuandtidade) {
    precoTotalCarrinho +=
      ...
      idsProdutoCarrinhoQuandtidade[idProdutoNoCarrinho];
  }
